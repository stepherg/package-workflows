...
def build_package(package_name: str, workflow_file: Path, platform: str, docker_platform: str, timeout_seconds: int = 600, output_file=None) -> bool:
    """
    Build a single package using Docker directly (not act).
        ...
        if output_file:
            ...
            with open(output_file, 'a') as log:
                ...
                result = subprocess.run(
                    ...
                    cwd=workflow_file.parent.parent
                )
        else:
...
def main():
    ...
    for idx, package_name in enumerate(build_order, 1):
        ...
        if build_package(package_name, workflow, platform, docker_platform, timeout_seconds, log_file):
